    ...
    <script>
      ...
      if (points.length === 0) {
        ...
        const layers = points.map((point) => {
          ...
          marker.bindPopup(point.label || "Marker");
          return marker;
        });
    ...
    try {
      ...
      const points = (data ?? [])
        ...
        .map((item, index) => ({
          latitude: Number(item.latitude),
          longitude: Number(item.longitude),
          label: item.user_id ? `User ${item.user_id}` : `Marker ${index + 1}`,
        }));

      setDbMarkers(points);
    } catch {
      Alert.alert("Map data error", "Unable to load saved coordinates.");
    }
  };
